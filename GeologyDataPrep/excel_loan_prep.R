# Prep FV loans from Excel exports for EMu import
# 2023-Sep-7, FMNH-IT
...
# Import loan data ####
# NOTE:
# If the read_excel functions have encoding or parsing errors,
# check that the script's filenames & extensions are correct.

raw_dh_invoices <- read_excel("GeologyDataPrep/raw_data/DH - Invoices.xls")

invoice_list_full <- read_excel("GeologyDataPrep/raw_data/DH - Invoice List - full li.xls")

# Check imported data ####
#   - raw_dh_invoices
#       - We expect warnings in this dataframe since 
...
if (NROW(not_in_raw_dh) > 0) {
  write_csv(not_in_raw_dh,
            "GeologyDataPrep/real_output/loan_invoices_not_in_raw_dh_fromExcel.csv", 
            na = "")
}

write_csv(loan_invoices, 
          "GeologyDataPrep/real_output/loan_invoices_fromExcel.csv", 
          na = "")

write_csv(loan_objects_prepped, 
          "GeologyDataPrep/real_output/loan_objects_fromExcel.csv", 
          na = "")
